...
if __name__ == "__main__":
    radius = 0.05 # 0.05       ??????
    act_distance = 2.0 # 0.4   ??????
    voxel_size = 0.03 #0.05   distance between block
    render_voxel_size = 0.009 #0.02  blocks size
    clipping_distance = 5.0 #0.7  depth of detection
    ...
    motion_gen_config = MotionGenConfig.load_from_robot_config(
        ...
        collision_checker_type=CollisionCheckerType.BLOX,
        use_cuda_graph=True,
        num_trajopt_seeds=12,#12
        num_graph_seeds=12,#12
        interpolation_dt=0.03,#0.03
        collision_activation_distance=0.025,#0.025
        acceleration_scale=1.0, # 1.0
        self_collision_check=True,
        maximum_trajectory_dt=0.1,#0.25
    ...
    if not args.use_debug_draw:
        voxel_viewer = VoxelManager(300, size=render_voxel_size)#100
    cmd_step_idx = 0
    while simulation_app.is_running():
        ...
        if cmd_plan is None and step_index % 10 == 0:
            ...
            cu_js = JointState(
                ...
                velocity=tensor_args.to_device(sim_js.velocities) * 0.0,#*0.0
                acceleration=tensor_args.to_device(sim_js.velocities) * 0.0,#*0.0
                jerk=tensor_args.to_device(sim_js.velocities) * 0.0,#*0.0
